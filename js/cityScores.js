(function () {
	...
	var attrArray = ["Overall Score", "Diesel (ppm)",
		"Asthma", "Cleanup Sites", "Low Birth Weight Pctl", "Pollution Burden", "Poverty"]
	...
	function setMap() {
		...
		Promise.all([d3.json(alameda),
			...
			createDropdown(cities, cityBounds, map, path, water, county);
		});
	...
	function enumerateRegions(input, map, path, csvData, colorScale) {
		...
		var desc = region.append("desc")
			.text('{"stroke": "white", "stroke-width": "0.1px"}');
		return region
	...
	function createDropdown(csvData, layer, map, path, water, county) {
		...
		var dropdown = d3.select("#filter")
			...
			.on("change", function () {
				changeAttribute(this.value, csvData, layer, map, path, water, county)
			});
	...
	function changeAttribute(attribute, csvData, layer, map, path, water, county) {
